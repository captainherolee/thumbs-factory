...
export default function PowerBarChart() {
  ...
  const data: DataEntry[] = [{ id: 1, name: '전력', power: electricityValue?.activePower ?? 0 }];
  ...
  return (
    <ResponsiveContainer width="100%" height={70}>
      <BarChart data={data} layout="vertical">
        ...
        <XAxis type="number" dataKey="power" domain={[0, ConvertBarGraphMax(electricityValue?.maximumDemand ?? 0)]} stroke="#fff" />
        <Bar
          dataKey="power"
          fill={
            data[0].power >= Math.floor(((electricityValue?.maximumDemand ?? 0) * (electricityValue?.mdAlarmSetValue ?? 0)) / 100) ? PointOverColor : PointColor
          }
        >
          {data.map((entry) => (
